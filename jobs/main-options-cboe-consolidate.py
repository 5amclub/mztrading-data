...
for optionsAssetUrl, stocksAssetUrl, name in last_30_entries:
  ...
  if match:
    ...
    try:
      ...
      if "404" in str(e):
        print(f"Warning: Options data not found for {date}. URL: {optionsAssetUrl}. Skipping.")
      else:
    ...
    try:
      ...
      if "404" in str(e):
        print(f"Warning: Stocks data not found for {date}. URL: {stocksAssetUrl}. Skipping.")
      else:
        raise  # Re-raise if it's not a 404 error
  else:
...
try:
  ...
  print(f"Successfully exported options data to {output_file}")
except Exception as e:
  print(f"Warning: Could not export options data: {str(e)}")
...
try:
  ...
  print(f"Successfully exported stocks data to {stocks_output_file}")
except Exception as e:
  print(f"Warning: Could not export stocks data: {str(e)}")
...
try:
  ...
  print(f"Warning: Could not process options file stats: {str(e)}")
...
try:
  ...
  """)
  print(f"Successfully created summary report: {summary_report_file}")
except Exception as e:
...
try:
  ...
  with open(summary_file, "w") as file:
      json.dump({
          "name": release_name, 
          "assetUrl":f"https://github.com/5amclub/mztrading-data/releases/download/{release_name}/options_cboe_rolling_30.parquet", 
          "stockUrl":f"https://github.com/5amclub/mztrading-data/releases/download/{release_name}/stocks_cboe_rolling_30.parquet",
          "greeksReportCsv":f"https://github.com/5amclub/mztrading-data/releases/download/{release_name}/all_symbols_summary_report.csv",
          "symbolsSummary": json.loads(symbols_summary)
      }, file, indent=4)

  print(f"Updated summary file: {summary_file}")
except Exception as e:
  print(f"Warning: Could not update summary file: {str(e)}")
